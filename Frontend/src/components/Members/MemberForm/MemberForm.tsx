...
const MemberForm = () => {
    ...
    const { data, isError, isFetching, isSuccess } = useGetAllRolesQuery();
    ...
    const onSubmit: SubmitHandler<FormFields> = async (data) => {
        try {
            ...
            setError("root", {
                message: "Something went wrong: Unable to add role"
            })
        }
    }
            ...
            <div className="input-container flex flex-col gap-3">
                ...
                <select {...register("role")} className="select select-bordered w-full">
                    ...
                    {isSuccess &&
                        <>
                            {data?.map((role: any) => (
                                <option key={role.id} value={role.userRole}>{role.userRole}</option>
                            ))}
                        </>
                    }
